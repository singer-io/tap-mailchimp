...
class MailchimpInterruptedSyncTest(MailchimpBaseTest):
    ...
    def test_run(self):
        ...
        start_date_datetime = self.parse_date(self.start_date, self.START_DATE_FORMAT)
        ...
        state = {
            ...
            "bookmarks": {"lists": {"8c775a04fb": {"list_members": {"datetime": "2022-08-11T06:24:35.000000Z"}}}},
        }
        ...
        for stream in expected_streams:
            with self.subTest(stream=stream):
                ...
                full_records = [
                    message["data"] for message in synced_records_full_sync.get(stream, {}).get("messages", [])
                ]
                interrupted_records = [
                    message["data"] for message in synced_records_interrupted_sync.get(stream, {}).get("messages", [])
                ]
                ...
                final_bookmark_value = self.get_bookmark(final_state.get("bookmarks"), stream_bookmark_path)
                # Gather Expectations
                expected_replication_key = list(expected_replication_keys[stream])[0]
                ...
                self.parse_date(str(final_bookmark_value), self.BOOKMARK_DATETIME_FORMAT)
                ...
                if stream == "list_members":
                    ...
                    interrupted_records = [rec for rec in interrupted_records if rec.get("list_id") == parent_id_value]
                    full_records = [rec for rec in full_records if rec.get("list_id") == parent_id_value]

                    # Check if the interrupted stream has a bookmark written for it
                    interrupted_bookmark_datetime = self.parse_date(
                        self.get_bookmark(state.get("bookmarks"), stream_bookmark_path), self.BOOKMARK_DATETIME_FORMAT
                    )
                    ...
                    for record in interrupted_records:
                        rec_time = self.parse_date(record.get(expected_replication_key), self.BOOKMARK_DATETIME_FORMAT)
