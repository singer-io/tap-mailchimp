...
class TestInit(unittest.TestCase):
    ...
    def test_do_discover_authentication_error(self):
        """Test case to verify we raise an exception when we get the error
        ...
        self.assertEqual(
            str(e.exception),
            "Error testing Mailchimp authentication. Error: Exception: `access_token` or `api_key` required",
        )
    ...
    def test_do_discover_catalog_generation(self, mocked_get, mocked_discover, mocked_json_dump):
        """Test case to verify we generate catalog on successful
        authentication."""
    ...
    def test_main(self, name, test_data, expected_data, mocked_sync, mocked_do_discover, mocked_parse_args):
        """Test case to verify we call discovery and sync as per the passed
        args."""
