...
class TestChunkSize(unittest.TestCase):
    ...
    def test_chunk_size_bookmark(self, name, test_data, expected_data, mocked_write_bookmark):
        ...
        reports_email_activities.write_email_activity_chunk_bookmark(
            current_bookmark=0, current_index=test_data[1], sorted_campaigns=sorted_campaigns
        )
    ...
    def test_chunk_size_valid(self, name, test_data, expected_data):
        ...
        chunk_campaigns = reports_email_activities.chunk_campaigns(sorted_campaigns=sorted_campaigns, chunk_bookmark=0)
    ...
    @parameterized.expand(
        [
            ["zero_int_chunk_size", {"chunk_size": 0}, "The chunk_size cannot be Zero(0)."],
            ["zero_string_chunk_size", {"chunk_size": "0"}, "The chunk_size cannot be Zero(0)."],
            ["negative_int_chunk_size", {"chunk_size": -5}, "The chunk_size cannot be negative."],
            ["negative_int_string_chunk_size", {"chunk_size": "-5"}, "The chunk_size cannot be negative."],
        ]
    )
    ...
    def test_chunk_size_error_with_logger(self, name, test_data, expected_data, mocked_logger_info):
        """Test cases to verify that an exception is raised and corresponding
        logger is called when zero value or negative integer value is passed to
        the chunk_size parameter in config."""
    ...
    def test_chunk_size_error_without_logger(self, name, test_data):
        ...
        value or string value is passed to the chunk_size parameter in
        config."""
