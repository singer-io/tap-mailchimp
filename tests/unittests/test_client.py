...
class TestClient(unittest.TestCase):
    ...
    config = {"user_agent": "test_agent", "access_token": "test_token", "api_key": "test_key", "request_timeout": 0}
    ...
    def test_get_base_url(self, mocked_request):
        """Test case to verify that the base_url is set depending upon the
        endpoint."""
    ...
    def test_request(self):
        ...
        resp = self.obj.request(method="GET", path=None, url="https://mailchimp.com", s3=True, endpoint="base_url")

        self.assertEqual(resp.status_code, 200)

    def test_mailchimp_client(self):
        """Test case to verify that the client is initialized appropriately
        depending upon the provided config."""

        test_config = {"user_agent": "test_agent", "request_timeout": 0, "api_key": "test_key", "dc": "test"}
    ...
    def test_request_no_url(self, mocked_logger, mocked_request, mocked_sleep):
        """Test case to verify working of request function when no url is
        provided but path is provided."""

        with self.assertRaises(requests.exceptions.Timeout):
            self.obj.request(method="GET", path="test/TEST")

        mocked_logger.info.assert_called_with(
            "Executing %s request to %s with params: %s", "GET", "https://test.com/3.0test/TEST", None
        )
    ...
    def test_request_no_url_no_path(self, mocked_logger, mocked_request, mocked_sleep):
        """Test case to verify the working of request function when neither url
        nor path is provided."""
        ...
        with self.assertRaises(requests.exceptions.Timeout):
            obj.request(method="GET", path="test/TEST")

        mocked_logger.info.assert_called_with(
            "Executing %s request to %s with params: %s", "GET", "https://login.mailchimp.com/oauth2/metadata", None
        )
    ...
    def test_request_no_s3_raises_exception(self):
        ...
        If not, then exception must be raised.
        """

        config = {"user_agent": "test_agent", "request_timeout": 0}
        obj = MailchimpClient(config=config)
