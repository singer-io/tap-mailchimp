...
class StreamsTest(unittest.TestCase):
    ...
    @parameterized.expand([["non_zero_value", 3], ["zero_value", 0]])
    def test_next_sleep_interval(self, name, test_value_1):
        """Test case to verify that the return value of next_sleep_interval is
        greater than or equal to the previous_sleep_interval.

        If previous_sleep_interval is not given (or zero is given) then
        it should take the default value specified.
        """
    ...
    @parameterized.expand([["zero_chunk_bookmark", 0, 3], ["non_zero_chunk_bookmark", 2, 1]])
    ...
    def test_chunk_campaigns_chunk_bookmark(self, name, test_value_1, test_value_2, mocked_logger):
        """Test case to verify that the chunk_campaigns function divides the
        provided sorted_campaigns starting from the provided chunk_bookmark."""
        ...
        chunk_campaigns = stream.chunk_campaigns(sorted_campaigns=sorted_campaigns, chunk_bookmark=test_value_1)
    ...
    @parameterized.expand(
        [
            [
                ...
                [{"campaign_id": "test1", "email_id": "test2", "TEST_key": "TEST_val"}],
            ],
            [
                "empty_activity",
                [{"campaign_id": "test2", "email_id": "test2", "activity": [], "_links": [{"test_key": "test_val"}]}],
                [],
            ],
        ]
    )
    def test_transform_activities(self, name, test_value1, test_value2):
        """Test case to verify that 'activity' field in records is transformed
        correctly."""
        # Mailchimp client
        client = MailchimpClient(config={})
    ...
    def test_nested_set(self):
        """Test case to verify that in nested dictionaries, key provided at
        given path should be initialized with the given value."""
    ...
    def test_get_path(self, name, stream, expected_path):
        """Test case to verify that path is returned appropriately as per the
        given arguments."""
    ...
    def test_process_records(self, name, initial_bookmark, expected_bookmark):
        """Test case to verify that the records are processed and correct
        bookmark is returned depending upon the initial bookmark and the
        records."""
        ...
        test_record = [{"last_changed": "2008-02-01"}, {"last_changed": "2008-02-10"}, {"last_changed": "2008-02-02"}]

        actual_bookmark = obj.process_records(records=test_record, max_bookmark_field=initial_bookmark)
    ...
    def test_incremental_sync(self, mocked_get_bookmark, mocked_basestream_sync):
        """Test case to verify the working of sync function for incremental
        stream."""
    ...
    def test_write_activity_batch_bookmark(self, mocked_write_bookmark):
        """Test case to verify that bookmark is written for
    ...
    @parameterized.expand([["test1", 500], ["test2", 25]])
    @mock.patch("tap_mailchimp.streams.BaseStream.write_bookmark")
    def test_write_email_activity_chunk_bookmark(self, name, test_value1, mocked_write_bookmark):
        """Test case to verify that bookmark is written for chunks of campaigns
        in 'reports_email_activity' stream."""
    ...
    def test_get_batch_info(self, mocked_client_get):
        """Test case to verify that error is raised when incorrect batch_id is
        provided."""

        _object_ = streams.ReportEmailActivity
        mocked_client_get.side_effect = HTTPError("HTTP Error: Not authorized.", response=MockResponse(401))
    ...
    def test_get_batch_info_batch_expire_error(self, mocked_client_get):
        """Test case to verify that the 'BatchExpiredError' error is raised
        when we encounter a 404 error."""

        _object_ = streams.ReportEmailActivity

        mocked_client_get.side_effect = HTTPError("HTTP Error: Not Found.", response=MockResponse(404))
    ...
    def test_check_and_resume_email_activity_batch(
        self, mocked_logger, mocked_get_bookmark, mocked_get_batch_info, mocked_sync_email_activities
    ):
        """Test case to verify that a batch is checked and resumed depending
        upon the bookmark."""
        ...
        mocked_logger.assert_called_with(
            "reports_email_activity - Previous run from state (%s) is empty, retrying.", "8vh837xfqd"
        )
    ...
    @mock.patch("tap_mailchimp.streams.BaseStream.get_bookmark")
    @mock.patch("tap_mailchimp.streams.LOGGER.info")
    def test_check_and_resume_email_activity_batch_batchexpired_error(
        self, mocked_logger, mocked_get_bookmark, mocked_get_batch_info, mocked_sync_email_activities
    ):
        """Test case to verify that a batch is checked and resumed depending
        upon the bookmark."""
        ...
        mocked_logger.assert_called_with("reports_email_activity - Previous run from state expired: %s", "8vh837xfqd")
    ...
    def test_poll_email_activity(self, mocked_logger, mocked_get_batch_info, mocked_write_activity_batch_bookmark):
        """Test case to verify that all required operations are executed for
        each batch and the progress is calculated accordingly."""
        ...
        expected_data = {"id": "test", "status": "finished", "total_operations": 2, "finished_operations": 2}

        actual_data = _object_.poll_email_activity(batch_id="8vh837xfqd")

        self.assertEqual(actual_data, expected_data)

    @parameterized.expand([["no_batch_id", None, 2], ["batch_id", "8vh837xfqd", 1]])
    ...
    def test_sync_email_activities(
        self,
        name,
        test_value_1,
        test_value_2,
        mocked_stream_email_activity,
        mocked_poll_email_activity,
        mocked_write_activity_batch_bookmark,
        mocked_post,
        mocked_format_selected_fields,
    ):
        """Test case to verify that all batches of campaigns for
        'reports_email_activity' stream are synced and bookmark is written for
        the same."""
    ...
    def test_sync_report_activities(
        self, mocked_write_email_activity_chunk_bookmark, mocked_sync_email_activities, mocked_write_bookmark
    ):
        """Test case to verify that 'reports_email_activity' stream is
        synced."""
    ...
    def test_sync(
        self,
        mocked_sync_report_activities,
        mocked_sync_substream,
        mocked_write_bookmark,
        mocked_format_selected_fields,
        mocked_client_get,
        ...
        mocked_client_get.return_value = {"test": [{"campaigns": [], "total_items": 0, "_links": []}]}
