...
ERROR_CODE_EXCEPTION_MAPPING = {
    400: {"raise_exception": MailchimpBadRequestError, "message": "Mailchimp Client faced a bad request."},
    401: {"raise_exception": MailchimpUnAuthorizedError, "message": "The API key is either invalid or disabled."},
    403: {
        "raise_exception": MailchimpForbiddenError,
        "message": "User does not have access to the requested operation.",
    },
    404: {"raise_exception": MailchimpNotFoundError, "message": "The requested resource could not be found."},
    405: {
        "raise_exception": MailchimpMethodNotAllowedError,
        "message": "The resource does not accept the HTTP method.",
    },
    414: {"raise_exception": MailchimpNestingError, "message": "The sub-resource requested is nested too deeply."},
    422: {
...
def raise_for_error(response):
    """Function to raise an error by extracting the message from the error
    response."""
    try:
    ...
    msg = json_response.get("detail", ERROR_CODE_EXCEPTION_MAPPING.get(status_code, {}).get("message", "Unknown Error"))
...
def retry_pattern(fnc):
    ...
    @backoff.on_exception(
        backoff.expo, (Server5xxError, MailchimpRateLimitError, ConnectionError, Timeout), max_tries=6, factor=3
    )
    @functools.wraps(fnc)
...
class MailchimpClient:
    def __init__(self, config):
        ...
        if not self.__access_token and self.__api_key:
            self.__base_url = "https://{}.api.mailchimp.com".format(config.get("dc"))
    ...
    def get_base_url(self):
        data = self.request("GET", url="https://login.mailchimp.com/oauth2/metadata", endpoint="base_url")
