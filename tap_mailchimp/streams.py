...
from singer.utils import should_sync_field, strptime_to_utc
...
def transform_activities(records):
    """Function to move activity at the top-level from the email activity
    records."""
...
class BaseStream:
    ...
    def process_records(self, records, max_bookmark_field=None, sync_start=None):
        """Function to transform and write records and get the maximum bookmark
        value."""
        stream = self.stream
        schema = stream.schema.to_dict()
        ...
        with metrics.record_counter(self.stream_name) as counter, Transformer() as transformer:
            for record in records:
                ...
                if sync_start and record[self.replication_keys[0]] < sync_start:
                    continue
                if self.replication_keys and (
                    max_bookmark_field is None or record[self.replication_keys[0]] > max_bookmark_field
                ):
    ...
    def format_selected_fields(self):
        """Given a catalog with selected metadata return a comma separated
        string of the `data_key.field_name` for every selected field in the
        catalog plus the `extra_fields` defined below.
    ...
    def sync_substream(self, child, parent_record_id):
        """Function to update the API URL based on parent id and sync child
        stream."""
        child_stream_obj = STREAMS.get(child)(
            self.state, self.client, self.config, self.catalog, self.selected_stream_names, self.child_streams_to_sync
        )
    ...
    def sync(self, sync_start_date=None):
        ...
        offset = 0
        has_more = True
        params = {"count": page_size, **self.params}
        while has_more:
            ...
            LOGGER.info(
                ...
                self.stream_name,
                f"since: {last_datetime}, " if self.bookmark_query_field else "",
                page_size,
                offset,
            )
            ...
            data = self.client.get(self.path, params=params, endpoint=self.stream_name)
            ...
            for report_stream in self.report_streams:
                ...
                if report_stream not in self.selected_stream_names:
                    continue
                reports_stream_obj = STREAMS.get(report_stream)(
                    self.state,
                    self.client,
                    self.config,
                    self.catalog,
                    self.selected_stream_names,
                    self.child_streams_to_sync,
                )
...
class Lists(FullTable):
    ...
    key_properties = ["id"]
    path = "/lists"
    params = {"sort_field": "date_created", "sort_dir": "ASC"}
...
class Campaigns(FullTable):
    ...
    key_properties = ["id"]
    path = "/campaigns"
    params = {"status": "sent", "sort_field": "send_time", "sort_dir": "ASC"}
...
class ReportEmailActivity(Incremental):
    ...
    def __init__(self, state, client, config, catalog, selected_stream_names, child_streams_to_sync):
        """Override the __init__ and set 'chunk_size' class variable to use
    ...
    def chunk_campaigns(self, sorted_campaigns, chunk_bookmark):
        """Function to break list for sorted campaigns into the batch size and
        send the chunked list."""
        chunk_start = chunk_bookmark * self.chunk_size
        chunk_end = chunk_start + self.chunk_size

        if chunk_bookmark > 0:
            LOGGER.info(
                "reports_email_activity - Resuming requests starting at campaign_id %s (index %s) in chunks of %s",
        ...
        while not done:
            ...
            if not done:
                ...
                LOGGER.info(
                    "reports_email_activity - Will request for campaign_ids from %s to %s (index %s to %s)",
                    sorted_campaigns[chunk_start],
    ...
    def get_batch_info(self, batch_id):
        ...
        try:
            return self.client.get(f"/batches/{batch_id}", endpoint="get_batch_info")
        ...
        if batch_id:
            try:
                ...
                if data["status"] == "finished" and not data["response_body_url"]:
                    LOGGER.info("reports_email_activity - Previous run from state (%s) is empty, retrying.", batch_id)
                    return
            except BatchExpiredError:
                LOGGER.info("reports_email_activity - Previous run from state expired: %s", batch_id)
                return
    ...
    def poll_email_activity(self, batch_id):
        ...
        while True:
            ...
            LOGGER.info("reports_email_activity - Job polling: %s - %s%s", data["id"], data["status"], progress)

            if data["status"] == "finished":
                return data
            if (time.time() - start_time) > MAX_RETRY_ELAPSED_TIME:
                message = "Mailchimp campaigns export is still in progress after {} seconds. \
                    Will continue with this export on the next sync.".format(
            ...
            LOGGER.info("campaigns - status: %s, sleeping for %s seconds", data["status"], sleep)
            time.sleep(sleep)

    def stream_email_activity(self, archive_url):
        """Get the Email Activities from the provided URL after batch
        completion."""
                ...
                while file:
                    if file.isfile():
                        ...
                        for i, operation in enumerate(operations):
                            ...
                            LOGGER.info(
                                "reports_email_activity - [batch operation %s] Processing records for campaign %s",
                                i,
                                campaign_id,
                            )
                            ...
                            if operation["status_code"] != 200:
                                ...
                                max_bookmark_field = self.process_records(
                                    self.transform_activities(email_activities), last_bookmark
                                )
    ...
    def sync_email_activities(self, campaign_ids, batch_id=None):
        """Sync email activities ie.

        create the batch, get response URL and fetch data from the
        response URL
        """
        if batch_id:
                ...
                operations.append(
                    {
                        ...
                        "params": {"since": since, "fields": formatted_field_names},
                    }
                )
        ...
        LOGGER.info(
            "reports_email_activity - Batch job complete: took %.2fs minutes",
    ...
    def sync_report_activities(self, campaign_ids):
        """Function to loop over the chunk of campaigns and sync email
        activities."""
